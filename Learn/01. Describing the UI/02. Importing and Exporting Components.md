...
- 컴포넌트의 가장 큰 장점은 재사용성으로 컴포넌트를 조합해 또 다른 컴포넌트를 만들 수 있다는 점입니다.
...
- 루트 컴포넌트 파일은 무엇인가?
- 컴포넌트의 `export`, `import`
- `default`, `named` `export`, `import`의 사용
...
- 랜딩 화면을 과학책으로 변경하거나 프로필 사진을 다른 곳에서 사용하게 된다면 `Gallery` 컴포넌트와 `Profile` 컴포넌트를 `root` 컴포넌트가 아닌 다른 파일로 옮기는 게 좋습니다.
- 그렇게 변경하면 재사용성이 높아 컴포넌트를 모듈화 할 수 있습니다.
- 컴포넌트를 다른 파일로 이동하는 세 가지 단계가 있습니다.

1. 컴포넌트를 추가할 `JS` 파일을 생성합니다.
2. 새로 만든 파일에서 함수 컴포넌트를 `export` 합니다. (`default` 또는 `named` `export` 방식을 사용합니다)
3. 컴포넌트를 사용할 파일에서 `import` 합니다. (적절한 방식을 선택해서 `default` 또는 `named`로 `import` 합니다)
...
- `JavaScript`에서는 `default`와 `named`라는 두 가지 방법으로 `export` 합니다.
- 지금까지는 `default export`만 사용했지만 두 방법 모두 1개 파일에서 사용할 수 있습니다.
- 다만 1개 파일에서는 1개 `default export`만 존재할 수 있고 `named exports`는 **여러 개**가 존재할 수 있습니다.
...
- 반대로 `named import`를 사용할 때는 양쪽 파일의 이름이 **일치해야 하므로** `named import`라고 불립니다.
...
- 하지만 `Gallery.js` 파일에는 이미 하나의 `default export`가 존재하기 때문의 2 개의 `default export`를 정의할 수 없습니다.
...
- 루트 컴포넌트 파일이란?
- 컴포넌트를 `import`하거나 `export`하는 방법
- `default` 및 `named` `import` 및 `export`를 사용하는 때와 방법
- 동일한 파일에서 여러 컴포넌트를 `export`하는 방법

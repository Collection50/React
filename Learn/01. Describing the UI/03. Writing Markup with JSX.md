...
- 반대로, **버튼**의 마크업과 **사이드바**의 마크업처럼 서로 관련이 없는 항목끼리는 자체적으로 변경하는 방법이 더 안전하기 때문에 분리되도록 합니다.
  <br><br>
...
> `JSX`와 `React`는 서로 다른 개념이기에, 각각 독립적으로 사용 가능합니다.
...
- 한 컴포넌트에서 여러 엘리먼트를 반환하려면, 1개 상위 태그로 감싸야 합니다.
...
### Deep Dive - 여러 개의 JSX를 래핑해야 하는 이유는 뭔가요?

- `JSX`는 `HTML`처럼 보이지만 내부적으로는 일반 `JS` 객체로 변환됩니다.
- 배열로 감싸지 않은 하나의 함수는 2개의 객체를 반환할 수 없습니다.
- 따라서 또 다른 태그나 *fragment*로 감싸지 않으면 2개 이상의 `JSX`를 반환할 수 없습니다.
...
- 예를 들면, 변수명에 대시를 포함하거나 `class`처럼 예약어를 사용할 수 없습니다.
  <br><br>
- 이것이 바로 `React`에서 `HTML`과 `SVG` 어트리뷰트의 대부분이 **카멜 케이스**로 작성되는 이유입니다.
- 예를 들어 `stroke-width` 대신 `strokeWidth`로 사용합니다.
...
> 역사적 이유로, `aria-*`와 `data-*`의 어트리뷰트는 `HTML`과 동일하게 대시를 사용하여 작성합니다.
...
- 컨버터를 사용하여 `HTML`과 `SVG`를 `JSX`로 변환하는 것을 추천합니다.
- 컨버터는 매우 유용하지만 그래도 `JSX`를 어떻게 쓰는지 이해하는 것도 중요합니다.
...
- 필요한 경우 컨버터를 사용할 수 있습니다.
- 오류 메시지는 마크업을 수정하는 올바른 방향을 알려줍니다.
